...
def test_transaction():
    ...
    assert  transaction.output[sender_wallet.address] == sender_wallet.ballance - amount

    assert 'timestamp' in transaction.input
    assert transaction.input['amount'] == sender_wallet.ballance
...
def test_transaction_update():
    ...
    first_amount = 50
    transaction = Transaction(sender_wallet, first_amount, first_amount)  
    ...
    assert transaction.output[sender_wallet.address] ==\
          sender_wallet.ballance - first_amount - next_amount 
    assert Wallet.verify(
        transaction.input['public_key'],
        transaction.output,
        transaction.input['signature']
    ...
    assert transaction.output[sender_wallet.address] ==\
        sender_wallet.ballance - first_amount - next_amount - to_first_again_amount
    assert Wallet.verify(
        transaction.input['public_key'],
        transaction.output,
        transaction.input['signature']
...
def test_Invalid_reward_transaction_extra_recipient(): 
    ...
    with pytest.raises(Exception,match='Invalid Rewards'):
        Transaction.is_valid_transaction(reward_transaction)  
